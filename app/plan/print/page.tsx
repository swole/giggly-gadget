  ...
  return (
    <main className="print-sheet relative z-10 mx-auto max-w-3xl px-6 py-8 text-[var(--color-ink)]">
      ...
      <header className="flex items-baseline justify-between border-b-2 border-[var(--color-ink)] pb-2">
        ...
        <span className="noprint text-[10px] uppercase tracking-[0.18em] text-[var(--color-muted)]">Ctrl/Cmd + P to print</span>
      </header>

      <table className="mt-4 w-full border-collapse text-[12px]">
        <thead>
          <tr className="text-left text-[9px] uppercase tracking-[0.16em] text-[var(--color-muted)]">
        ...
        <div className="mt-3 grid grid-cols-2 gap-x-8 gap-y-4 text-[12px] sm:grid-cols-3">
          {SHOP_ORDER.filter((s) => byShop.has(s)).map((s) => (
            <section key={s} className="break-avoid">
              <h3 className="text-[10px] uppercase tracking-[0.18em] text-[var(--color-terra)]">{SHOP_LABEL[s]}</h3>
              {CATEGORY_ORDER.filter((c) => byShop.get(s)!.has(c)).map((c) => (
                <div key={c} className="mt-1.5">
                  <div className="text-[9px] uppercase tracking-[0.14em] text-[var(--color-muted)]">{CATEGORY_LABEL[c]}</div>
                  <ul>
                        ...
                        <span>
                          {r.qty_min !== null && (
                            <span className="tabular-nums">
                              {renderQty(Number(r.qty_min))}
                              {r.qty_max !== null && r.qty_max !== r.qty_min ? `–${renderQty(Number(r.qty_max))}` : ""}{" "}
                            </span>
                          )}
      ...
      <footer className="mt-8 text-[9px] uppercase tracking-[0.16em] text-[var(--color-faint)]">
