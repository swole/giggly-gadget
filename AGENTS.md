...
| Route | What |
|---|---|
| `/` | **Kitchen** - today's meals (big cards: Cook â–¸ â†’ cook mode, Recipe, Mark cooked with an 8 s Undo / tap-to-un-mark, notes for the cook, progress ring), tomorrow (+ "Tonight" prep hints), the rest of the week. Refreshes itself when the SG date rolls over. `/kitchen` redirects here. |
| `/plan?week=YYYY-MM-DD` | Week planner Monâ€“Sat (+Sun collapsed as rest day). Weeks start blank; the header **Randomize** die (and the die on each day card) opens the themed RandomizeSheet - theme chips (added by / heart healthy / cuisine / â‰¤30 min / want to try / â˜…4+, remembered in `localStorage` `gg-roll-theme-v1`), live match counts, fill-empty vs re-roll, "Roll those again". A die sits on every *empty* slot for an instant one-slot roll (toast: Again / Undo); â‹¯ on a chip = note / **Pick another** (themed swap, "Put back") / open / remove (Undo). Slot â†’ picker (search, chips, Leftovers tab, and one-offs: type anything â†’ 'Add "white rice" as a one-off', recent one-offs as one-tap chips in `localStorage` `gg-recent-customs-v1`). **Share** builds a WhatsApp-ready week digest + `?as=helper` link (Web Share API, clipboard fallback). Tap J+L to cycle eaters. Constraint chips (oily fish â‰¥3, chicken â‰¤1, prawn/seafood â‰¤2), per-day protein J/L, â‹¯ Actions (fill from rotation, copy last week - "replace" asks first; **Clear the week**, cooked kept), Print. |
...
| `/recipes`, `/recipes/[id]`, `/recipes/[id]/cook` | Discover (paginated cards, was `/`), detail (household Both/Johnny/Lydia scale mode; `?eaters=&pm=` from the planner; **Plan this** sheet â†’ day/slot/eaters; `?new=1` opens it as a nudge after create; "Watch the video" header pill when `source_url` is a video; a "Learn how to make it" CTA card after the Method links the source video/recipe), cook mode (posts `planned_meal_id`; timer tray stacks above the footer; audio context created on the timer tap). |
